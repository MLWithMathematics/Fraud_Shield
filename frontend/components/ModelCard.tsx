...
import { ArrowRight, Cpu, ImageIcon, BarChart3 } from "lucide-react";
...
export default function ModelCard({ model, index }: Props) {
  ...
  const TypeIcon = model.type === "image" ? ImageIcon : Cpu;
    ...
    >
      ...
      <div className="p-6">
        ...
        <div className="flex items-start justify-between mb-4">
          ...
          >
            <TypeIcon className="w-4 h-4" style={{ color: accent.color }} />
          </div>
        </div>

        {/* Tagline */}
        <p className="text-sm text-gray-500 leading-relaxed mb-5">{model.tagline}</p>
        ...
        <div className="flex flex-wrap gap-1.5 mb-5">
          {model.techStack.map((tech) => (
            <span
              key={tech}
              className="text-[10px] font-mono px-2 py-0.5 rounded bg-white/4 border border-white/7 text-gray-500 hover:text-gray-300 transition-colors"
            >
              {tech}
            </span>
          ))}
          ...
          {model.stats.slice(0, 2).map((stat) => (
            <div key={stat.label} className="bg-white/3 rounded-lg px-3 py-2">
              <p
                ...
                style={{ color: stat.highlight ? accent.color : "#e5e7eb" }}
              >
                {stat.value}
              </p>
        ...
        <div className="flex items-center gap-2 mb-5">
          ...
          <span className="text-xs text-gray-600 font-mono">
            Input: {model.type === "numeric" ? "Structured / CSV data" : "Document image upload"}
          </span>
        </div>
